#!/usr/bin/env node
// Generate every Dataverket mark asset from one definition.
//
...
const BOX = [0, 0, 848, 1167];        // the viewBox the mark alone is drawn in
const INK = [29, 120, 820, 1147];     // what it actually covers: x0 y0 x1 y1
const INK_H = INK[3] - INK[1];

// Wordmark cap-height as a fraction of the mark's ink height, and the gap
// between them in the same unit. Reviewed by eye at 48 and 64 px: the rook is
// bottom-heavy, so a wordmark matched to its cap-height reads undersized.
...
// 'Dataverket' as outlines, placed to the right of the mark.
function wordmark() {
  ...
  // The word's cap box and the mark's ink share a centre line.
  ...
  // data-text marks the part that is lettering rather than art. Designsystemet
  // does the same on its own wordmark, so a theme can fill the text from a
